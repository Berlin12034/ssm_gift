	    ...
	    layui.use([ 'jquery', 'layer', 'form', 'table'  ], function() {
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'<%=basePath%>/customer/loadAllCustomer.action' //数据接口
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"<%=basePath%>/customer/loadAllCustomer.action?"+params ,
				    page:{
				    	curr:1
				    }
				})
			...
			//导出
				$("#doExport").click(function(){
					...
					window.location.href="<%=basePath%>/stat/exportCustomer.action?"+params;
				});
		   ...
		   table.on('tool(customerTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.custname+'】这个客户吗', function(index){
				       //向服务端发送删除指令
				       $.post("<%=basePath%>/customer/deleteCustomer.action",{identity:data.identity},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function openUpdateCustomer(data){
				mainIndex=layer.open({
					...
					success:function(index){
						...
						url="<%=basePath%>/customer/updateCustomer.action";
					}
				});
			}
			//保存
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些客户吗', function(index){
				       //向服务端发送删除指令
				       $.post("<%=basePath%>/customer/deleteBatchCustomer.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
