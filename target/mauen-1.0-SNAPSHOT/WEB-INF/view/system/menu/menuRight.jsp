...
<head>
	...
	<link rel="stylesheet" href="<%=basePath%>/resources/layui_ext/dtree/dtree.css">
	<link rel="stylesheet" href="<%=basePath%>/resources/layui_ext/dtree/font/dtreefont.css">
	<%--初始化下拉树的样式--%>
	<style type="text/css">
...
<body class="childrenBody">
	...
	<script type="text/javascript">
		/* var tableIns;把它提出来，让定义在外面函数的方法可以用*/
	    var tableIns;
	    layui.extend({
			dtree:'<%=basePath%>/resources/layui_ext/dist/dtree'
		  ...
		  * 请求url*/
			 tableIns=table.render({
			    ...
			    ,url:'<%=basePath%>/menu/loadAllMenu.action' //数据接口
		...
		/*templet:function(d)是后台返回的json数据，因为毒镖显示不出来，用这样的方法加载，用来做简单的html处理*/
			...
			$("#doSearch").click(function(){
				...
				alert(params);
				/*tableIns是加载返回的json数据*/
				/*tableIns.reload是重新加载url返回的数据,是加上了查询条件重新加载*/
				tableIns.reload({
					url:"<%=basePath%>/menu/loadAllMenu?"+params
				})
			});
		   ...
		   table.on('tool(menuTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  //先判断当前菜单有没有子节点
				  $.post("<%=basePath%>/menu/checkMenuHasChildren.action?id="+data.id,function(obj){
					  /*查出来的数据条数obj.code*/
				  	if(obj.code>=0){
						  ...
						  layer.confirm('真的删除【'+data.title+'】这个菜单吗', function(index){
						       //向服务端发送删除指令
						       $.post("<%=basePath%>/menu/deleteMenu",{id:data.id},function(res){
						    	   layer.msg(res.msg);
						    	    //刷新数据 表格
									tableIns.reload();
									//刷新左边的树
			   ...
			   } else if(layEvent === 'edit'){ //编辑调用修改方法data是上面选中的行
			     openUpdateMenu(data);
			   }
			 });
			...
			function openAddMenu(){
				mainIndex=layer.open({
					type:1,
					title:'添加菜单',
		          /*content:$("#saveOrUpdateDiv"),绑定了saveOrUpdateDiv里的内容，可以获取*/
					...
					success:function(index){
						...
						url="<%=basePath%>/menu/addMenu.action";
					}
				});
			}
			//打开修改页面
			function openUpdateMenu(data){
				mainIndex=layer.open({
					...
					success:function(index){
						...
						url="<%=basePath%>/menu/updateMenu.action";
						//反选下拉树,打开会匹配好下拉树的父类在父菜单上
						var pid=data.pid;
						/*给选中父节点的值赋值，定位到选中树的位置*/
					    var params = dtree.dataInit("menuTree", pid);
					    //移除菜单树打开的样式，当再次打开添加时候在没有点击输入框选择菜单时候就出现菜单树
					    $("#menuSelectDiv").removeClass("layui-show");
			...
			form.on("submit(doSubmit)",function(obj){
				//序列化表单数据，url是上面打开添加里要执行的路径,params是获取获取了dataFrm表单里的数据
			...
			var menuTree = dtree.render({
		          ...
		          url: "<%=basePath%>/menu/loadMenuManagerLeftTreeJson.action?spread=1",  // 使用url加载（可与data加载同时存在）跟获取左边的菜单栏一样
				  icon: "2",
				  accordion:true
				});
			/*监听菜单树输入框，再把它显示出来*/
				...
				dtree.on("node(menuTree)", function(obj){
				  ...
				  /*给一个被隐藏的input赋值，为了把当前被点击的pid带回过去*/
		...
		function reloadTable(id){
			alert(id);
			tableIns.reload({
		   /*id字段要一至，这样到后台就直接付给了id*/
				url:"<%=basePath%>/menu/loadAllMenu?id="+id
			})
		}
	</script>
</body>
