		...
		<form class="layui-form"  lay-filter="dataFrm" id="dataFrm">
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
					...
					<input type="text" name="rolename"  placeholder="请输入角色名称" autocomplete="off" lay-verify="required"
						class="layui-input">
				</div>
			</div>
			<div class="layui-form-item">
				<label class="layui-form-label">角色备注:</label>
				<div class="layui-input-block">
					<input type="text" name="roledesc" placeholder="请输入角色备注" autocomplete="off" lay-verify="required"
						class="layui-input">
				</div>
			</div>
			<div class="layui-form-item">
	    ...
	    layui.extend({
			...
			//渲染数据表格
			 tableIns=table.render({
			    ...
			    ,url:'<%=basePath%>/role/loadAllRole.action' //数据接口
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"<%=basePath%>/role/loadAllRole.action?"+params
				})
			});
		   ...
		   table.on('tool(roleTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.rolename+'】这个角色吗', function(index){
				       //向服务端发送删除指令
				       $.post("<%=basePath%>/role/deleteRole.action",{roleid:data.roleid},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function deleteBatch(){
			    ...
			    $.each(data,function(i,item){
			    	if(i==0){/*输出params是ids=1这样，i=0就是为了去掉第一个有&，ids=1&ids=2&ids=3*/
			    ...
			    layer.confirm('真的删除选中的这些角色吗', function(index){
				       //向服务端发送删除指令
				       $.post("<%=basePath%>/role/deleteBatchRole.action",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
				...
				mainIndex=layer.open({
					...
					yes: function (index, layero) {
						...
						$.each(nodes, function (i, item) {
							/*整条数据比如roleid=1&ids=2&ids=3ids是初始化树中被选中拥有那些菜单管理的id*/
							params += "&ids=" + item.nodeId;
						})
						//保存角色和菜单的关系
						$.post("<%=basePath%>/role/saveRoleMenu.action", params, function (obj) {
							layer.msg(obj.msg);
						})
					},
					success:function(index){
					   ...
					   menuTree = dtree.render({
			              ...
			              checkbarData: "choose" ,
				          url: "<%=basePath%>/role/initRoleMenuTreeJson.action?roleid="+data.roleid // 使用url加载（可与data加载同时存在）
					    });
					}
				});
			}
